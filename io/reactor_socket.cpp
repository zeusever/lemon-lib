...
LEMON_IO_API 
	...
	__lemon_inout LemonErrorInfo *errorCode)
{
	LemonIOEventQ Q = socket->Service->Q;
	...
	E->Type = LEMON_MAKE_FLAG(LEMON_IO_CONNECT_OP,LEMON_IO_WRITE_OP);

	LemonIOMultiplexerRegister(socket->Service->Mutiplexer,socket->Handle,E,errorCode);

	LemonRegisterIOEvent(socket->Service->Q,socket->Handle,E,errorCode);

	if(LEMON_FAILED(*errorCode)) LemonReleaseIOEvent(Q,E);
}
	...
	__lemon_inout LemonErrorInfo *errorCode)
{
	...
	E->Address.From.addr = addr; E->Address.From.len = addrlen;

	E->CallBack.Accept = callback; E->UserData = userData;

	E->Type = LEMON_MAKE_FLAG(LEMON_IO_ACCEPT_OP,LEMON_IO_READ_OP);
	...
	__lemon_inout LemonErrorInfo *errorCode)
{
	...
	if(LEMON_FAILED(*errorCode)) return;
	// init the io event
	E->Address.To.addr = address; E->Address.To.len = addressSize;

	E->CallBack.RW = callback; E->UserData = userData;

	E->Buffer.Write = buffer; E->BufferLength = bufferSize;

	E->Flags = flags;

	E->Type = LEMON_MAKE_FLAG(LEMON_IO_SENDTO_OP,LEMON_IO_WRITE_OP);
	...
	__lemon_inout LemonErrorInfo *errorCode)
{
	...
	E->Type = LEMON_MAKE_FLAG(LEMON_IO_SEND_OP,LEMON_IO_WRITE_OP);
	...
	__lemon_inout LemonErrorInfo *errorCode)
{
	...
	if(LEMON_FAILED(*errorCode)) return;
	// init the io event
	E->Address.From.addr = address; E->Address.From.len = addressSize;

	E->CallBack.RW = callback; E->UserData = userData;

	E->Buffer.Read = buffer; E->BufferLength = bufferSize;

	E->Flags = flags;

	E->Type = LEMON_MAKE_FLAG(LEMON_IO_RECVFROM_OP,LEMON_IO_READ_OP);
	...
	__lemon_inout LemonErrorInfo *errorCode)
{
	...
	E->Type = LEMON_MAKE_FLAG(LEMON_IO_RECV_OP,LEMON_IO_READ_OP);
